...
<script>
    ...
    var $_layoutWidth = parseInt(groupPlayerWidth);//1280
    var $_layoutHeight = parseInt(groupPlayerHeight);//720
    var $_maxZIndex = -1;
...
//            2 สร้างใหม่จากหน้า จะมีคำว่า gen- นำหน้า ตามด้วยเลย count
            ...
            createLayout(newLayoutId, "Screen", 0, 0, widthInit, heightInit, null);
            
            var $trId = "row-" + newLayoutId;
            createRow($trId, "Screen", 0, 0, widthInit, heightInit, null);
        });
...
//        $obj.css({top: y, left: x, width: width, height: height});
    ...
    var setPosition = function($obj, y, x) {
        ...
        var terminalX = parseInt(x / $_resWidth);
        var terminalY = parseInt(y / $_resHeight);
        $obj.css({top: terminalY, left: terminalX});
    };

    var setSize = function($obj, width, height) {
        var terminalWidth = parseInt(width / $_resWidth);
        var terminalHeight = parseInt(height / $_resHeight);
        $obj.css({width: terminalWidth, height: terminalHeight});
    };
...
//วิธีคือ เอาโหนดแม่หารสอง ลบ โหนดลูกหารสอง
    ...
    var createRow = function(id, name, top, left, width, height, zindex) {
        ...
        $tr.attr("id", id).append($tdName.attr("name", "tmn_name").html(name))
        .append($tdLeft.attr("name", "tmn_left").html(left))
        .append($tdTop.attr("name", "tmn_top").html(top))
        .append($tdWidth.attr("name", "tmn_width").html(width))
        .append($tdHeight.attr("name", "tmn_height").html(height))
        .append($tdZindex.attr("name", "tmn_zindex").html(zindex))
    ...
    var bindEventRow = function(){
        $(".trigger-td-input").unbind().bind("click", function(e){ 
            ...
            if(tdName == "tmn_left" || tdName == "tmn_width"){
                setSpinner($this, 0, $_layoutWidth);
            } else if(tdName == "tmn_top" || tdName == "tmn_height"){
                setSpinner($this, 0, $_layoutHeight);
            }else if(tdName == "tmn_zindex"){
        ...
        $(document).bind("click", function(e1){ 
            ...
            $("table tbody").find("tr").each(function(e2){
                ...
                if($isRefresh == true){
                    ...
                    var $name = $(this).find("td[name=tmn_name]").html();
                    var $left = $(this).find("td[name=tmn_left]").html();
                    var $top = $(this).find("td[name=tmn_top]").html();
                    var $width = $(this).find("td[name=tmn_width]").html();
                    var $height = $(this).find("td[name=tmn_height]").html();
                    var $zIndex = $(this).find("td[name=tmn_zindex]").html();
    ...
    var refreshLayout = function(id, name, top, left, width, height, zindex) {
        var $screen = $($_containerId).find("#" + id);
        $screen.find("span.middle").html(name);
        bindEvent("#" + id);
        setPosition($screen, top, left);
        setSize($screen, width, height);
        setZIndex($screen, zindex);
    };

    var refreshRow = function(id, name, top, left, width, height, zindex) {
        width = parseInt(width);
        height = parseInt(height);
        top = parseInt(top);
        left = parseInt(left);
        $("#row-"+ id).find("td[name=tmn_width]").html((width < 0 ? 0 : width));
        $("#row-"+ id).find("td[name=tmn_height]").html((height < 0 ? 0 : height));
        $("#row-"+ id).find("td[name=tmn_top]").html((top < 0 ? 0 : top));
        $("#row-"+ id).find("td[name=tmn_left]").html((left < 0 ? 0 : left));
    };
...
//                printoffset($div);
    ...
    var printSize = function($divId, ui) {
        ...
        var terminalWidth = parseInt(width * $_resWidth);
        var terminalHeight = parseInt(height * $_resHeight);
        $("#row-"+ $divId).find("td[name=tmn_width]").html((terminalWidth < 0 ? 0 : terminalWidth));
        $("#row-"+ $divId).find("td[name=tmn_height]").html((terminalHeight < 0 ? 0 : terminalHeight));
...
// x = ui.left;
            ...
            var terminalTop = parseInt(y * $_resHeight);
            var terminalLeft = parseInt(x * $_resWidth);
        $("#row-"+ $divId).find("td[name=tmn_top]").html((terminalTop < 0 ? 0 : terminalTop));
        $("#row-"+ $divId).find("td[name=tmn_left]").html((terminalLeft < 0 ? 0 : terminalLeft));
//        $("#row-"+ $divId).find("td[name=tmn_top]").html(y);
//        $("#row-"+ $divId).find("td[name=tmn_left]").html(x);
//            $("#printX").html(y);
//            $("#printY").html(x);
        }
...
<div>
    ...
    <table cellpadding="0" cellspacing="0" border="0" class="terminal groceryCrudTable" id="<?php echo $uniqid; ?>" style="border: 2px solid #E2E4FF;">
        <thead >
            <tr>
                ...
                <tr id='row-<?php echo $row->tmn_ID ?>'>
                    ...
                    <?php if (!$unset_delete || !$unset_edit || !$unset_read || !empty($actions)) { ?>
                        <td class='actions'>
                            ...
                            <?php } ?>
                            <?php if (!$unset_delete) { ?>
                                <a onclick = "javascript: return delete_row('<?php echo $row->delete_url ?>', '<?php echo $row->tmn_ID ?>')"
...
//                    var newLayoutId = "layout" + (++$_layoutCount);
                    ...
                    $(function(){
                        ...
                        var tmnId = '<?php echo $row->tmn_ID ?>';
                        var tmnName = '<?php echo $row->tmn_name ?>';
                        var tmnTop = (<?php echo $row->tmn_top ?> == 0 ? 0 : <?php echo $row->tmn_top ?>);
                        var tmnLeft = (<?php echo $row->tmn_left ?> == 0 ? 0 : <?php echo $row->tmn_left ?>);
                        var tmnWidth = (<?php echo $row->tmn_width ?> == 0 ? (parseInt(groupPlayerWidth) / 2) : <?php echo $row->tmn_width ?>);
                        var tmnHeight = (<?php echo $row->tmn_height ?> == 0 ? (parseInt(groupPlayerHeight) / 2) : <?php echo $row->tmn_height ?>);
                        
                        createLayout(tmnId, tmnName, tmnTop, tmnLeft, tmnWidth, tmnHeight, 0);
                                <?php // echo $row->tmn_zindex ?>
                        bindEventRow();
                        refreshRow(tmnId, tmnName, tmnTop, tmnLeft, tmnWidth, tmnHeight, 0);
                    });
//                    $("")
                </script>
