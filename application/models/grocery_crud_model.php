...
class grocery_CRUD_Model  extends CI_Model  {
    ...
    function get_relation_n_n_selection_array($primary_key_value, $field_info)
    {
    	...
    	foreach($results as $row)
    	{
            if($CI->router->class === "playlist"){
                $results_array[$row->{$field_info->primary_key_alias_to_selection_table}.'/'.
                            $row->{$field_info->priority_field_relation_table}] = $row->{$field_name_hash};
            }else{
