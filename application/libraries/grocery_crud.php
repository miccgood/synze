...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	{
            $var = $this->theme_path.$this->theme.'/views/list_template.php';
		if(!file_exists($var))
		{
	...
	protected function insert_layout($insert_result = false)
	{
		...
		else
		{
			...
			if(!$this->unset_back_to_list && !empty($insert_result) && !$this->unset_edit)
			{
                            ...
                            $CI = &get_instance();  
//                
//                            $CI->config->load('media', true);
//                            $this->media = $CI->config->item('media');
                            $dpmId = $CI->session->userdata("dpm_id");
                            if($dpmId !== false && $dpmId !== null){
                                $strDpmId = "?dpm_ID=$dpmId";
                            }
				$success_message .= " <a class='go-to-edit-form' href='".$this->getEditUrl($insert_result) . $strDpmId. "'>".$this->l('form_edit')." {$this->subject}</a> ";
	...
	protected function get_enum_input($field_info,$value)
	{
		...
		$options_array = $field_info->extras[0] !== false && is_array($field_info->extras[0])? $field_info->extras[0] : explode("','",substr($field_info->db_max_length,1,-1));
	...
	protected function get_relation_n_n_input($field_info_type, $selected_values)
	{
		...
		else
		{
			...
			if(!empty($unselected_values))
				...
				{
//					$input .= "<option value='$id'> $id : $name</option>";
			...
			if(!empty($selected_values))
				...
				{
//					$input .= "<option value='$id' selected='selected'>$id : $name</option>";
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		$file_display_none  	= empty($value) ?  "display:none;" : "";
                ...
                $is_video = $is_image = $is_text = FALSE;
...
//                        ."</video>  <script type=\"text/javascript\"> $(function(){ $('#video').css({'z-index':0}); });</script>";
                    ...
                    $input .= "<a href='".$file_url."' id='file_$unique' class='open-file";
                    if($is_image){
                        $input .= " $image_class'><img src='".$file_url."' height='240px'>";
                    }else if($is_text){
	...
	protected function _theme_view($view, $vars = array(), $return = FALSE)
	{
		...
		if (file_exists($view_file.$controller.$file))
		{
			$path = $view_file.$controller.$file;
			$file_exists = TRUE;
		}
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	protected $subject_plural 		= null;
        protected $default_value                = array();     // Add By momo
...
//					die();
	...
	public function set_subject( $subject )
	{
        ...
        public function set_default_value( $default_value = array() )
	{
	 ...
	 * @param mixed $where_clause
	 */
	public function set_relation_n_n($field_name, $relation_table, $selection_table, $primary_key_alias_to_this_table, 
                $primary_key_alias_to_selection_table , $title_field_selection_table , $priority_field_relation_table = null, 
                $where_clause = null, $custom_order_by = array(), $selected_values = array())
	{
